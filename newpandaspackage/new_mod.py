...
def train_val_test_func(df):
    ...
    train, test = train_test_split(
        df, train_size=.9, test_size=.1, random_state=42)

    # train and val split
    train, val = train_test_split(
        train, train_size=.8, test_size=.2, random_state=42)
...
def add_state_names(lst):
    ...
    # Can use 'Dir(new_frame['abbrev'])' to see what functions/methods you can
    # call on the particle datatype you're using
...
class StateWrangle():
    ...
    # Defining the function that turns a list into a series then into a
    # DataFrame
    def list_series_df_col(self):
        # Creating a dictionary with the zipped lists and column header
        # attributes
        dictionary = dict(
            zip(self.col_headers, [self.lst1, self.lst2, self.lst3]))
        # breakpoint() -> code check
